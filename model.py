...
class Individual(mvc.Listenable):
    ...
    def __init__(self, kind: str, region: "Population", row: int, col: int):
        ...
        super().__init__()
        # kind is how it access the parameter in the configuration file.
        self.kind = kind
        self.region = region
        self.row = row
        self.col = col
...
class Wanderer(Individual):
    ...
    def hello(self, vistior: "Individual") -> bool:
        return True
...
class AtRisk(Individual):
    ...
    def hello(self, vistior: "Individual") -> bool:
        return vistior in self.neighbors
...
class Population(mvc.Listenable):
    """This class is a subclass of Listenable"""
    def __init__(self, nrows: int, ncols: int):
        ...
        for row_i in range(config.get_int("Grid", "Rows")):
            ...
            for col_i in range(config.get_int("Grid", "Cols")):
                row.append(self._random_individual(row_i, col_i)) #row.append(Typical(self, row_i, col_i))  what is this doing? self?
    ...
    def step(self):
        ...
        for row in self.cells:
            for cell in row:
                cell.step() #calculates the next state for that cell
        for row in self.cells:
            for cell in row:
                cell.tick() #updating the current state to new state
    ...
    def seed(self):
        ...
        self.cells[row][col].infect() #choose random people to infect
    ...
    def _random_individual(self, row: int, col: int) -> "Individual":
        """creates either a Typical or an AtRisk, or Wanderer individual randomly"""
        ...
        while True:
            for the_class, proportion in classes:
                dice = random.random()
                if dice < proportion:
                    return the_class(self, row, col) # what does this self do?
    ...
    def neighbors(self, num: int, row: int, col: int, dist: int) -> List[Tuple[int, int]]:
        ...
        result = []
        count = 0
        log.debug(f"Cell {row},{col} finding {num} neighbors at distance {dist} " + f"in {self.nrows},{self.ncols}")
        attempts = 0
        while count < num:
            attempts += 1
